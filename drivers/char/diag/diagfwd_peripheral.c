...
static void
diagfwd_cntl_open (struct diagfwd_info *fwd_info)
{
  if (!fwd_info)
    return;
...
static void
diagfwd_cntl_close (struct diagfwd_info *fwd_info)
{
  if (!fwd_info)
    return;
...
static void
diagfwd_dci_open (struct diagfwd_info *fwd_info)
{
  if (!fwd_info)
    return;
...
static void
diagfwd_dci_close (struct diagfwd_info *fwd_info)
{
  if (!fwd_info)
    return;
...
static int
diag_add_hdlc_encoding (unsigned char *dest_buf, int *dest_len,
			unsigned char *buf, int len)
{
...
static int
check_bufsize_for_encoding (struct diagfwd_buf_t *buf, uint32_t len)
{
  int i, ctx = 0;
  uint32_t max_size = 0;
	  ...
	  if (driver->logging_mode == DIAG_MEMORY_DEVICE_MODE)
	    {
	      ...
	      for (i = 0; ch != NULL && i < ch->num_tbl_entries; i++)
		{
		  if (ch->tbl[i].buf == buf->data)
		    {
		      ...
		      DIAG_LOG (DIAG_DEBUG_PERIPHERALS,
				...
				GET_BUF_PERIPHERAL (ctx), GET_BUF_TYPE (ctx));
		      break;
		    }
		}
	    }
...
static void
diagfwd_data_process_done (struct diagfwd_info *fwd_info,
			   struct diagfwd_buf_t *buf, int len)
{
  ...
  if (hdlc_disabled)
	...
	{
	  pr_err
	    ("diag: In %s, no match for non encode buffer %pK, peripheral %d, type: %d\n",
	     __func__, buf, fwd_info->peripheral, fwd_info->type);
	  goto end;
	}
      if (len > PERIPHERAL_BUF_SZ)
	{
	  pr_err
	    ("diag: In %s, Incoming buffer too large %d, peripheral %d, type: %d\n",
	     __func__, len, fwd_info->peripheral, fwd_info->type);
	  goto end;
	}
      write_len = len;
      if (write_len <= 0)
  ...
  if (write_len > 0)
    {
      err = diag_mux_write (DIAG_LOCAL_PROC, write_buf, write_len, buf->ctxt);
      if (err)
	{
	  pr_err_ratelimited
	    ("diag: In %s, unable to write to mux error: %d\n", __func__,
	     err);
	  goto end;
	}
    }
  ...
  if (driver->feature[peripheral].encode_hdlc &&
      ...
      if (fwd_info->buf_1 && fwd_info->buf_1->data_raw == buf)
	{
	  ...
	  if (fwd_info->type == TYPE_DATA)
	    {
	      temp_buf_upd_1 = fwd_info->buf_upd_1_a->data_raw;
	      if (peripheral == PERIPHERAL_LPASS)
		temp_buf_upd_2 = fwd_info->buf_upd_2_a->data_raw;
	    }
	}
      else if (fwd_info->buf_2 && fwd_info->buf_2->data_raw == buf)
	{
	  ...
	  if (fwd_info->type == TYPE_DATA)
	    temp_buf_upd_1 = fwd_info->buf_upd_1_b->data_raw;
	  if (peripheral == PERIPHERAL_LPASS)
	    temp_buf_upd_2 = fwd_info->buf_upd_2_b->data_raw;
	}
      else
	{
	  pr_err
	    ("diag: In %s, no match for buffer %pK, peripheral %d, type: %d\n",
	     __func__, buf, peripheral, fwd_info->type);
	  goto end;
	}
      while (processed < len)
	{
	  buf_len = *(uint16_t *) (temp_buf_main + 2);
	  switch ((*temp_buf_main))
	    ...
	    case DIAG_ID_MPSS:
	      ...
	      if (temp_buf_cpd)
		{
		  memcpy (temp_buf_cpd, (temp_buf_main + 4), buf_len);
	    ...
	    case DIAG_ID_WLAN:
	      ...
	      if (temp_buf_upd_1)
		{
		  memcpy (temp_buf_upd_1, (temp_buf_main + 4), buf_len);
	    ...
	    case DIAG_ID_SENSORS:
	      ...
	      if (temp_buf_upd_2)
		{
		  memcpy (temp_buf_upd_2, (temp_buf_main + 4), buf_len);
      ...
      if (flag_buf_1)
	{
	  ...
	  if (peripheral == PERIPHERAL_LPASS && fwd_info->type == TYPE_DATA)
	  ...
	  fwd_info->type == TYPE_DATA && len_upd_2)
	{
	  ...
	  temp_ptr_upd->ctxt |= (SET_PD_CTXT (ctxt_upd_2));
	  atomic_set (&temp_ptr_upd->in_busy, 1);
	  diagfwd_data_process_done (fwd_info, temp_ptr_upd, len_upd_2);
	}
      else
	{
	  if (flag_buf_1)
      ...
      if (fwd_info->type == TYPE_DATA && len_upd_1)
	{
	  ...
	  temp_ptr_upd->ctxt &= 0x00FFFFFF;
	  temp_ptr_upd->ctxt |= (SET_PD_CTXT (ctxt_upd_1));
	  atomic_set (&temp_ptr_upd->in_busy, 1);
	  diagfwd_data_process_done (fwd_info, temp_ptr_upd, len_upd_1);
	}
      else
	{
	  if (flag_buf_1)
      ...
      if (len_cpd)
	{
	  ...
	  temp_ptr_cpd->ctxt |= (SET_PD_CTXT (ctxt_cpd));
	  diagfwd_data_process_done (fwd_info, temp_ptr_cpd, len_cpd);
	}
      else
	{
	  if (flag_buf_1)
...
static void
diagfwd_data_read_done (struct diagfwd_info *fwd_info,
			unsigned char *buf, int len)
{
  int err = 0;
  int write_len = 0;
...
static void
diagfwd_cntl_read_done (struct diagfwd_info *fwd_info,
			unsigned char *buf, int len)
{
  if (!fwd_info)
    {
...
static void
diagfwd_dci_read_done (struct diagfwd_info *fwd_info,
		       unsigned char *buf, int len)
{
  if (!fwd_info)
    return;
...
static void
diagfwd_reset_buffers (struct diagfwd_info *fwd_info, unsigned char *buf)
{
  if (!fwd_info || !buf)
    return;
...
int
diagfwd_cntl_register (uint8_t transport, uint8_t peripheral, void *ctxt,
...
int
diagfwd_register (uint8_t transport, uint8_t peripheral, uint8_t type,
...
void
diagfwd_deregister (uint8_t peripheral, uint8_t type, void *ctxt)
{
  struct diagfwd_info *fwd_info = NULL;
...
void
diagfwd_close_transport (uint8_t transport, uint8_t peripheral)
{
  struct diagfwd_info *fwd_info = NULL;
...
void *
diagfwd_request_write_buf (struct diagfwd_info *fwd_info)
{
  void *buf = NULL;
  int index;
  unsigned long flags;
...
int
diagfwd_write (uint8_t peripheral, uint8_t type, void *buf, int len)
{
  struct diagfwd_info *fwd_info = NULL;
  int err = 0;
  ...
  if (type == TYPE_CMD || type == TYPE_DCI_CMD)
	  ...
	  !driver->feature[peripheral].sent_feature_mask)
	{
	  pr_debug_ratelimited
	    ("diag: In %s, feature mask for peripheral: %d not received or sent yet\n",
	     __func__, peripheral);
	  return 0;
	}
  ...
  else if (fwd_info->transport == TRANSPORT_GLINK)
    diagfwd_write_buffer_done (fwd_info, buf_ptr);
  return err;
}

static void
__diag_fwd_open (struct diagfwd_info *fwd_info)
{
  if (!fwd_info)
    return;
...
void
diagfwd_early_open (uint8_t peripheral)
{
  uint8_t transport = 0;
  struct diagfwd_info *fwd_info = NULL;
...
void
diagfwd_open (uint8_t peripheral, uint8_t type)
{
  struct diagfwd_info *fwd_info = NULL;
...
void
diagfwd_late_open (struct diagfwd_info *fwd_info)
{
  __diag_fwd_open (fwd_info);
}

void
diagfwd_close (uint8_t peripheral, uint8_t type)
{
  struct diagfwd_info *fwd_info = NULL;
...
int
diagfwd_channel_open (struct diagfwd_info *fwd_info)
{
  int i;
  if (!fwd_info)
    return -EIO;
...
int
diagfwd_channel_close (struct diagfwd_info *fwd_info)
{
  int i;
  if (!fwd_info)
    return -EIO;
...
int
diagfwd_channel_read_done (struct diagfwd_info *fwd_info,
...
void
diagfwd_write_done (uint8_t peripheral, uint8_t type, int buf_num)
{
  struct diagfwd_info *fwd_info = NULL;
  ...
  if (buf_num == 1 && fwd_info->buf_1)
    {
      ...
      if (peripheral == PERIPHERAL_LPASS)
	{
	  if (!fwd_info->upd_len_1_a && !fwd_info->upd_len_2_a)
	    atomic_set (&fwd_info->buf_1->in_busy, 0);
	}
  ...
  else if (buf_num == 2 && fwd_info->buf_2)
    {
      ...
      if (peripheral == PERIPHERAL_LPASS)
	{
	  if (!fwd_info->upd_len_1_b && !fwd_info->upd_len_2_b)
	    atomic_set (&fwd_info->buf_2->in_busy, 0);
	}
      else if (peripheral == PERIPHERAL_MODEM)
	{
      ...
      if (peripheral == PERIPHERAL_LPASS)
	{
	   ...
	   */
	  if (!fwd_info->cpd_len_1 && !fwd_info->upd_len_2_a)
	    atomic_set (&fwd_info->buf_1->in_busy, 0);
	}
      else
	{
      ...
      if (peripheral == PERIPHERAL_LPASS)
	{
	   ...
	   * free the core pd buffer for LPASS
	   */
	  if (!fwd_info->cpd_len_2 && !fwd_info->upd_len_2_b)
	    atomic_set (&fwd_info->buf_2->in_busy, 0);
	}
      else
	{
  ...
  else if (buf_num == 5 && fwd_info->buf_upd_2_a && fwd_info->buf_1)
    {
       ...
       * free the core pd buffer for LPASS
       */
      if (!fwd_info->cpd_len_1 && !fwd_info->upd_len_1_a)
	{
	  atomic_set (&fwd_info->buf_1->in_busy, 0);
	  DIAG_LOG (DIAG_DEBUG_PERIPHERALS,
		    "Buffer 1 for core PD is marked free, p: %d, t: %d, buf_num: %d\n",
  ...
  else if (buf_num == 6 && fwd_info->buf_upd_2_b && fwd_info->buf_2)
    {
       ...
       * free the core pd buffer for LPASS
       */
      if (!fwd_info->cpd_len_2 && !fwd_info->upd_len_1_b)
	{
	  atomic_set (&fwd_info->buf_2->in_busy, 0);
	  DIAG_LOG (DIAG_DEBUG_PERIPHERALS,
		    "Buffer 2 for core PD is marked free, p: %d, t: %d, buf_num: %d\n",
...
int
diagfwd_write_buffer_done (struct diagfwd_info *fwd_info, const void *ptr)
{
...
void
diagfwd_channel_read (struct diagfwd_info *fwd_info)
{
  int err = 0;
  uint32_t read_len = 0;
  ...
  if (!fwd_info->inited || !atomic_read (&fwd_info->opened))
	...
	("diag: In %s, p: %d, t: %d, inited: %d, opened: %d  ch_open: %d\n",
	 __func__, fwd_info->peripheral, fwd_info->type, fwd_info->inited,
	 atomic_read (&fwd_info->opened), fwd_info->ch_open);
      diag_ws_release ();
      return;
    }
  ...
  if (fwd_info->buf_1 && !atomic_read (&fwd_info->buf_1->in_busy))
    {
      if (driver->feature[fwd_info->peripheral].encode_hdlc &&
	  (fwd_info->type == TYPE_DATA || fwd_info->type == TYPE_CMD))
	{
...
static void
diagfwd_queue_read (struct diagfwd_info *fwd_info)
{
  if (!fwd_info)
    return;
...
void
diagfwd_buffers_init (struct diagfwd_info *fwd_info)
{
  ...
  if (!fwd_info->buf_1)
    {
      fwd_info->buf_1 = kzalloc (sizeof (struct diagfwd_buf_t), GFP_KERNEL);
  ...
  if (fwd_info->type == TYPE_DATA)
      ...
      if (!fwd_info->buf_2->data)
	{
	  ...
	  fwd_info->buf_2->ctxt = SET_BUF_CTXT (fwd_info->peripheral,
						fwd_info->type, 2);
	}
      ...
      if (driver->feature[fwd_info->peripheral].untag_header)
	{
	  if (!fwd_info->buf_upd_1_a)
		...
		kzalloc (sizeof (struct diagfwd_buf_t), GFP_KERNEL);
	      if (ZERO_OR_NULL_PTR (fwd_info->buf_upd_1_a))
		goto err;
	      kmemleak_not_leak (fwd_info->buf_upd_1_a);
	    }

	  if (fwd_info->buf_upd_1_a && !fwd_info->buf_upd_1_a->data)
	    {
	      fwd_info->buf_upd_1_a->data =
		kzalloc (PERIPHERAL_BUF_SZ + APF_DIAG_PADDING, GFP_KERNEL);
		...
		goto err;
	      ...
	      fwd_info->buf_upd_1_a->ctxt =
		SET_BUF_CTXT (fwd_info->peripheral, fwd_info->type, 3);
	    }
	  ...
	  if (fwd_info->buf_upd_1_b && !fwd_info->buf_upd_1_b->data)
	    {
	      fwd_info->buf_upd_1_b->data =
		kzalloc (PERIPHERAL_BUF_SZ + APF_DIAG_PADDING, GFP_KERNEL);
	      temp_char_buf = fwd_info->buf_upd_1_b->data;
	      if (ZERO_OR_NULL_PTR (temp_char_buf))
		goto err;
	      fwd_info->buf_upd_1_b->len = PERIPHERAL_BUF_SZ;
	      kmemleak_not_leak (temp_char_buf);
	      fwd_info->buf_upd_1_b->ctxt =
		SET_BUF_CTXT (fwd_info->peripheral, fwd_info->type, 4);
	    }
	  if (fwd_info->peripheral == PERIPHERAL_LPASS)
	      ...
	      if (!fwd_info->buf_upd_2_a)
		{
		  fwd_info->buf_upd_2_a =
		    kzalloc (sizeof (struct diagfwd_buf_t), GFP_KERNEL);
		  temp_fwd_buf = fwd_info->buf_upd_2_a;
	      ...
	      if (!fwd_info->buf_upd_2_a->data)
		{
		  fwd_info->buf_upd_2_a->data =
		    kzalloc (PERIPHERAL_BUF_SZ +
			     APF_DIAG_PADDING, GFP_KERNEL);
		  temp_char_buf = fwd_info->buf_upd_2_a->data;
		  if (ZERO_OR_NULL_PTR (temp_char_buf))
		    goto err;
		  fwd_info->buf_upd_2_a->len = PERIPHERAL_BUF_SZ;
		  kmemleak_not_leak (temp_char_buf);
		  fwd_info->buf_upd_2_a->ctxt =
		    SET_BUF_CTXT (fwd_info->peripheral, fwd_info->type, 5);
		}
	      if (!fwd_info->buf_upd_2_b)
		{
		  fwd_info->buf_upd_2_b =
		    kzalloc (sizeof (struct diagfwd_buf_t), GFP_KERNEL);
		  temp_fwd_buf = fwd_info->buf_upd_2_b;
		  if (ZERO_OR_NULL_PTR (temp_fwd_buf))
		    goto err;
		  kmemleak_not_leak (temp_fwd_buf);
		}
	      ...
	      if (!fwd_info->buf_upd_2_b->data)
		{
		  fwd_info->buf_upd_2_b->data =
		    kzalloc (PERIPHERAL_BUF_SZ +
			     APF_DIAG_PADDING, GFP_KERNEL);
		  temp_char_buf = fwd_info->buf_upd_2_b->data;
		  if (ZERO_OR_NULL_PTR (temp_char_buf))
		    goto err;
		  fwd_info->buf_upd_2_b->len = PERIPHERAL_BUF_SZ;
		  kmemleak_not_leak (temp_char_buf);
		  fwd_info->buf_upd_2_b->ctxt =
		    SET_BUF_CTXT (fwd_info->peripheral, fwd_info->type, 6);
		}
	    }
	}
      ...
      if (driver->supports_apps_hdlc_encoding)
	{
	  ...
	  if (!fwd_info->buf_1->data_raw)
		...
		kzalloc (PERIPHERAL_BUF_SZ + APF_DIAG_PADDING, GFP_KERNEL);
	      temp_char_buf = fwd_info->buf_1->data_raw;
	      if (ZERO_OR_NULL_PTR (temp_char_buf))
		goto err;
	  ...
	  if (!fwd_info->buf_2->data_raw)
		...
		kzalloc (PERIPHERAL_BUF_SZ + APF_DIAG_PADDING, GFP_KERNEL);
	      temp_char_buf = fwd_info->buf_2->data_raw;
	      if (ZERO_OR_NULL_PTR (temp_char_buf))
		goto err;
	  ...
	  if (driver->feature[fwd_info->peripheral].untag_header)
	    {
	      if (fwd_info->buf_upd_1_a && !fwd_info->buf_upd_1_a->data_raw)
		{
		  fwd_info->buf_upd_1_a->data_raw =
		    kzalloc (PERIPHERAL_BUF_SZ +
			     APF_DIAG_PADDING, GFP_KERNEL);
		  temp_char_buf = fwd_info->buf_upd_1_a->data_raw;
		  if (ZERO_OR_NULL_PTR (temp_char_buf))
		    goto err;
		  fwd_info->buf_upd_1_a->len_raw = PERIPHERAL_BUF_SZ;
		  kmemleak_not_leak (temp_char_buf);
		}

	      if (fwd_info->buf_upd_1_b && !fwd_info->buf_upd_1_b->data_raw)
		{
		  fwd_info->buf_upd_1_b->data_raw =
		    kzalloc (PERIPHERAL_BUF_SZ +
			     APF_DIAG_PADDING, GFP_KERNEL);
		  temp_char_buf = fwd_info->buf_upd_1_b->data_raw;
		  if (ZERO_OR_NULL_PTR (temp_char_buf))
		    goto err;
		  fwd_info->buf_upd_1_b->len_raw = PERIPHERAL_BUF_SZ;
		  kmemleak_not_leak (temp_char_buf);
		}
	      if (fwd_info->peripheral == PERIPHERAL_LPASS
		  && !fwd_info->buf_upd_2_a->data_raw)
		{
		  fwd_info->buf_upd_2_a->data_raw =
		    kzalloc (PERIPHERAL_BUF_SZ +
			     APF_DIAG_PADDING, GFP_KERNEL);
		  temp_char_buf = fwd_info->buf_upd_2_a->data_raw;
		  if (ZERO_OR_NULL_PTR (temp_char_buf))
		    goto err;
		  fwd_info->buf_upd_2_a->len_raw = PERIPHERAL_BUF_SZ;
		  kmemleak_not_leak (temp_char_buf);
		}
	      if (fwd_info->peripheral == PERIPHERAL_LPASS
		  && !fwd_info->buf_upd_2_b->data_raw)
		{
		  fwd_info->buf_upd_2_b->data_raw =
		    kzalloc (PERIPHERAL_BUF_SZ +
			     APF_DIAG_PADDING, GFP_KERNEL);
		  temp_char_buf = fwd_info->buf_upd_2_b->data_raw;
		  if (ZERO_OR_NULL_PTR (temp_char_buf))
		    goto err;
		  fwd_info->buf_upd_2_b->len_raw = PERIPHERAL_BUF_SZ;
		  kmemleak_not_leak (temp_char_buf);
		}
	    }
	}
...
static void
diagfwd_buffers_exit (struct diagfwd_info *fwd_info)
{
...
void
diagfwd_write_buffers_init (struct diagfwd_info *fwd_info)
{
  unsigned long flags;
  int i;
  ...
  for (i = 0; i < NUM_WRITE_BUFFERS; i++)
    {
      if (!fwd_info->buf_ptr[i])
	fwd_info->buf_ptr[i] =
	  kzalloc (sizeof (struct diagfwd_buf_t), GFP_ATOMIC);
      if (!fwd_info->buf_ptr[i])
	goto err;
...
static void
diagfwd_write_buffers_exit (struct diagfwd_info *fwd_info)
{
  unsigned long flags;
  int i;
